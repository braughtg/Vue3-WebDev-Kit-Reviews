...
- Format the results of the review as follows:
  ...
  - Include one row in the table for each criteria in the custom sections.
- In the "Status" Column:
  - Use the character ✅ for "Status" if the primary and all secondary criteria given in parenthesis `(...)` is met.
  - Use the character ⚠️ for "Status" if the primary requirement is met but a secondary requirement is not met.
